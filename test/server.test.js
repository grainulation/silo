...
function json(response) {
  try {
    ...
    throw new Error(`Failed to parse JSON (status ${response.status}): ${response.body.slice(0, 200)}`);
  }
}
...
describe('silo server endpoints', () => {
  before(async () => {
    ...
    serverProcess.on('exit', (code) => {
      if (code && code !== 0 && code !== null) {
        process.stderr.write(`silo server exited with code ${code}\n${stderr}\n`);
      }
    });
    await waitForServer();
  });
  ...
  it('GET /events returns SSE stream', async () => {
    ...
    assert.ok(res.body.startsWith('data: '), 'should receive an SSE data frame');
  ...
  it('GET /api/docs returns 200 with HTML', async () => {
    ...
    assert.ok(res.body.includes('silo API'), 'docs page should mention silo API');
    assert.ok(res.body.includes('/health'), 'docs page should list /health endpoint');
  });
});
