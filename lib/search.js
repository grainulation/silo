...
class Search {
   ...
   * @param {string} opts.tier - Filter by evidence tier
   * @param {number} opts.limit - Max results (default 20)
   */
  query(query, opts = {}) {
    const { type, tier, limit = 20 } = opts;
    ...
    for (const file of files) {
      ...
      for (const claim of claims) {
        ...
        if (tier && claim.tier !== tier) continue;

        // Score by token matches across searchable fields
        const searchable = [
          claim.text || '',
          claim.type || '',
          claim.tags?.join(' ') || '',
