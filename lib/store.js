...
/**
 * Resolve the silo root directory, honoring explicit argument first,
 * then the `SILO_STORE` env var (set by the grainulator plugin so
 * plugin-managed silo data doesn't collide with the user's personal
 * ~/.silo), then falling back to the platform default.
 */
function resolveSiloDir(explicit) {
  if (explicit) return explicit;
  if (process.env.SILO_STORE) return path.resolve(process.env.SILO_STORE);
  return DEFAULT_SILO_DIR;
}

export class Store {
  constructor(siloDir) {
    siloDir = resolveSiloDir(siloDir);
    this.root = siloDir;
