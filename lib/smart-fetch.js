...
async function smartFetch(url, options = {}) {
	...
	const extractedSerialized = Buffer.byteLength(
		JSON.stringify({ title: meta.title, description: meta.description, content }),
		"utf8",
	);
	...
	const response = {
		...
		reduction_pct:
			fullBytes > 0 ? Math.round((1 - extractedSerialized / fullBytes) * 100) : 0,
		warnings,
	};

	// Cache successful fetches only (don't cache errors or failed quality).
	// Key by final_url to prevent redirect-based cache poisoning:
	// a malicious server redirecting attacker-url → target-url cannot
	// overwrite target-url's cached entry.
	if (cache && opts.cache && response.ok && quality !== "failed") {
		try {
			const cacheKey = response.final_url || url;
			cache.set(cacheKey, opts.mode, response);
		} catch (err) {
		...
		const req = client.get(
			...
			(res) => {
				...
				stream.on("data", (c) => {
					...
					if (
						!sawCloseTag &&
						/<\/main>|<\/article>/i.test(body.slice(-2048))
					) {
						sawCloseTag = true;
...
function extractMeta(html) {
	...
	const ldJsonCount = (html.match(/<script\s+type=["']application\/ld\+json["']/gi) || []).length;
